...
const MyList = ({ todos }) => {
  ...
  const updateTask = (event) => {
    ...
    const indexToUpdate = todoItemArray
      .map((item) => item.id)
      .indexOf(idToUpdate);
  ...
  // largest value found.
  const generateId = () => {
    return todoItemArray.reduce(
      (highest, index) => (index.id > highest ? index.id + 1 : highest + 1),
      0
    );
  };
  ...
  return (
    <s.StyledListContainer>
      <s.StyledToDoList>
        ...
        <s.StyledUl>{generateList}</s.StyledUl>
        <s.StyledForm>
